...
public class CategoryController {
    ...
    // 授權改由 DynamicAuthorizationManager 動態處理:此端點已登記於 resources 表,
    // 未授權角色會在進入 controller 前被擋 403,Service 完全不碰權限。
    @Operation(summary = "新增分類(限授權角色,名稱須唯一)")
    ...
    @Operation(summary = "修改分類名稱/排序(限授權角色,樂觀鎖防併發)")
    ...
    @Operation(summary = "刪除分類(限授權角色,軟刪除,樂觀鎖防併發)")
    ...
    // 刻意未登記於 resources 表:任何登入帳號都可查(未登入被擋 401)
